...
def main(
    ...
    Example:
        >>> # Basic usage with 2 joint chains (direct chain specification)
        >>> result, skin = main([
        ...
        ... )
    """
    creator = CreateLoftSurface(joint_chains)
...
def create_from_root_joints(
    ...
    """Create lofted surface from root joints.

    This function automatically traverses each root joint's hierarchy
    to build joint chains, then creates the lofted surface.

    Args:
        root_joints (list[str]): List of root joint names. At least 2 required.
            Each root joint's descendants will be collected as a chain.
        skip (int): Number of joints to skip in each chain.
        close (bool): Whether to close the loft loop.
        output_type (str): Type of output geometry. One of: 'nurbsSurface', 'mesh'.
        surface_divisions (int): Number of additional divisions between curves.
        center (bool): Whether to center cubic curves.
        curve_divisions (int): Number of CVs to insert between joint positions.
        is_bind (bool): Whether to create skin cluster and apply weights.
        weight_method (str): Weight calculation method.
        smooth_iterations (int): Number of weight smoothing iterations.
        parent_influence_ratio (float): Ratio of influence from parent joint.
        remove_end (bool): Merge end joint weights to parent.
        loft_weight_method (str): Loft direction weight distribution method.
        to_skin_cage (bool): Convert to skin cage mesh.
        skin_cage_division_levels (int): Division levels for skin cage.

    Returns:
        tuple[str, str | None]: (geometry_name, skin_cluster_name or None if not bound)

    Example:
        >>> # Automatically expand joint chains from root joints
        >>> result, skin = create_from_root_joints(
        ...     ["joint_A1", "joint_B1", "joint_C1"],
        ...     close=True,
        ...     is_bind=True,
        ... )
    """
    # Get joint chains from root joints
    joint_chains = get_joint_chains_from_roots(root_joints, skip)
...
def create_from_parallel_joints(
    parallel_rows: list[list[str]],
    close: bool = False,
    output_type: str = OUTPUT_NURBS_SURFACE,
    surface_divisions: int = 0,
    center: bool = False,
    ...
    """Create lofted surface from parallel joint rows.

    This function takes rows of parallel joints (same hierarchy level across chains)
    and transposes them into joint chains.

    Example input (parallel rows):
        Row 0: [jointAA, jointBA, jointCA]  <- same level (root)
        Row 1: [jointAB, jointBB, jointCB]  <- same level (child 1)
        Row 2: [jointAC, jointBC, jointCC]  <- same level (child 2)

    Transposed to chains:
        Chain 0: [jointAA, jointAB, jointAC]
        Chain 1: [jointBA, jointBB, jointBC]
        Chain 2: [jointCA, jointCB, jointCC]

    Args:
        parallel_rows (list[list[str]]): List of parallel joint rows.
            Each row contains joints at the same hierarchy level.
            All rows must have the same number of joints.
        close (bool): Whether to close the loft loop.
        output_type (str): Type of output geometry.
        surface_divisions (int): Number of additional divisions between curves.
        center (bool): Whether to center cubic curves.
        curve_divisions (int): Number of CVs to insert between joint positions.
        is_bind (bool): Whether to create skin cluster and apply weights.
    ...
    Example:
        >>> # Build chains from parallel selections
        >>> result, skin = create_from_parallel_joints(
        ...     [
        ...         ["jointAA", "jointBA", "jointCA"],  # Row 0 (roots)
        ...         ["jointAB", "jointBB", "jointCB"],  # Row 1
        ...         ["jointAC", "jointBC", "jointCC"],  # Row 2
        ...     ],
        ...     close=True,
        ...     is_bind=True,
        ... )
    """
    # Transpose parallel rows to joint chains
    joint_chains = transpose_parallel_joints(parallel_rows)
...
__all__ = [
    # Main functions
    "main",
    "create_from_root_joints",
    "create_from_parallel_joints",
    # Classes
    "CreateLoftSurface",
    "LoftWeightSetting",
    # Helper functions
