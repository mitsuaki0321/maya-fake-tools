...
4. 値のコピーは、属性を右クリックして表示されるメニューからも実行できます。
...
値のコピーは、属性を右クリックして表示されるメニューか、アクションバーの `Copy Value` ボタンから実行します。

![context](../../images/rig/connection_editor/context.png)

* **Copy Value** : 選択ペアの値をコピーします。
* **Copy Value (Leaf)** : 子属性ごとに値をコピーします。

コピー先に既に入力接続がある場合は、設定した値が無視されるため警告が表示されます。コピー先がロックされている場合は、`Options > Force connect` をオンにすると上書きできます。
