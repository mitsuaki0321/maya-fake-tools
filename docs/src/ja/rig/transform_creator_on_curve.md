...
2. シーン上の NURBS カーブ、サーフェース、またはサーフェースの isoparm を選択します。
...
- **UpVector**
  - 作成されるトランスフォームノードのアップベクトルを取得する方法を設定します。
    ...
    - **CurveNormal**
      - カーブの法線ベクトルを取得します。
    - **SurfaceNormal**
      - カーブが属するサーフェスの法線ベクトルを取得します。以下のいずれかの場合に使用できます。
        - サーフェースの isoparm を選択している場合
        - サーフェースを直接選択している場合
        - duplicateCurve コマンドで作成されたカーブを選択している場合
      - 上記に該当しない場合、CurveNormal にフォールバックします。
- **SurfaceDir**
